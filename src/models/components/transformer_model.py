...
class TransformerModel(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        activation = "relu"
        # activation = "gelu"
        self.project_inp = nn.Linear(self.n_features, self.d_model)
        self.pos_enc = get_pos_encoder(pos_encoding)(
            self.d_model,
            dropout=self.dropout * (1.0 - freeze),
            max_len=max_len,
        )
    ...
    def build_output_module(self):
        output_layer = nn.Sequential(
            nn.Linear(
                self.d_model * self.length_sequence, self.mlp_dim
            ),
            nn.ReLU(),
            self.dropout1,
            nn.Linear(self.mlp_dim, self.output_size),
        )
    ...
    def forward(self, X):
        ...
        inp = self.conv1(X)
        inp = self.conv2(inp) * math.sqrt(self.d_model)
